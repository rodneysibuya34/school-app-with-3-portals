...
export default function TeacherPortal() {
  ...
  const [submittedHomework, setSubmittedHomework] = useState<Record<number, string>>(() => {
    if (typeof window === 'undefined') return {};
    const stored = localStorage.getItem("submittedHomework");
    return stored ? JSON.parse(stored) : {};
  });
  ...
  const renderHomework = () => (
    <div>
      ...
      {Object.keys(submittedHomework).length > 0 && (
        <div className="mt-12">
  ...
  const renderStudents = () => {
    ...
    const filteredStudents = gradeFilter ? filteredByGrade.filter(s => s.grade === parseInt(gradeFilter)) : filteredByGrade;
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] flex flex-col md:flex-row">
      ...
      <main className="flex-1 p-3 md:p-8 overflow-y-auto w-full md:ml-0">
        <div className="md:hidden flex items-center justify-between mb-4 p-2 bg-stone-800 rounded-lg">
          ...
          <span className="text-white font-semibold">{navItems.find(n => n.label.toLowerCase() === activeTab)?.label || 'Teacher'}</span>
        </div>
